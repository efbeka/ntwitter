...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  var streamParser = new streamparser(),
      nativeStream = new Stream();

  nativeStream.readable = true;
  nativeStream.paused = false;
  nativeStream.buffer = [];

  nativeStream.destroySilent = function() {
    ...
    nativeStream.emit('close');
  };

  nativeStream.destroy = function() {
    // FIXME: should we emit end/close on explicit destroy?
    nativeStream.destroySilent();

    // emit the 'destroy' event
    nativeStream.emit('destroy','socket has been destroyed');
  };
  
  nativeStream.pause = function() {
    this.paused = true;
  };
  ...
  function flushBuffer(data) {
    processTweet(data);
  }
  
  nativeStream.resume = function() {
    async.forEach(nativeStream.buffer, flushBuffer, afterFlush);
  };

  streamParser.on('_data', function (tweet) {
    if (nativeStream.paused) {
      nativeStream.buffer.push(tweet);
      return;
    }
    processTweet(tweet);
  });
  ...
  function processTweet(tweet) {
    if (tweet['limit']) {
      nativeStream.emit('limit', tweet['limit']);
    }
    else if (tweet['delete']) {
      nativeStream.emit('delete', tweet['delete']);
    }
    else if (tweet['scrub_geo']) {
      nativeStream.emit('scrub_geo', tweet['scrub_geo']);
    }
    else {
      nativeStream.emit('data', JSON.stringify(tweet));
    }
  }
  ...
  request.on('response', function(response) {
    ...
    if(response.statusCode > 200) {
      nativeStream.destroySilent();
      nativeStream.emit('error', 'http', response.statusCode );
    }
    else
    {
      ...
      response.on('error', function(error) {
        nativeStream.emit('error', error);
      });
      response.on('end', function() {
        nativeStream.emit('end', response);
      });
       ...
       * socket. So, 'nativeStream' object will fire the 'destroy' event as we can see above.
       */
  ...
  if ( typeof callback === 'function' ) callback(nativeStream);
